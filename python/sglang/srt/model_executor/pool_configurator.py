...
def _operator_swa_full_tokens_ratio() -> Optional[float]:
    ...
    The bag value is the resolution result, which the DeepSeek-V4 model override
    fills in; the raw input is what the caller actually passed."""
    from sglang.srt.runtime_context import get_server_args

    raw = getattr(get_server_args(), "_raw_input", None)
    if raw is None:
        return get_schedule().swa_full_tokens_ratio
    return raw.get("swa_full_tokens_ratio")
...
class DSV4PoolConfigurator(MemoryPoolConfigurator):
    ...
    def _get_bytes_per_full_token(self) -> float:
        ...
        kv_bytes = self.attn_head_dim * 2 if self._unified else self.kv_bytes
        return (
            # Ring mode: SWA is a fixed per-request pool (see _fixed_swa_bytes).
            (0.0 if self._unified else swa_ratio * self.bytes_per_swa_token)
    ...
    def _get_swa_fixed_bytes(self) -> float:
        ...
        if self.swa_cap_tokens is None or self._unified:
            # Ring mode reserves the per-request SWA ring in _fixed_swa_bytes instead.
            return 0
